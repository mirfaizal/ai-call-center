...
class SummarizationAgent:
    ...
    def __init__(
        ...
        self.mcp_router = MCPRouter(api_key=self.api_key)
    ...
    def summarize(self, transcript: str, call_id: str = "unknown") -> CallSummary:
        ...
        result = self.mcp_router.invoke([{"role": "user", "content": prompt}], model=self.model)
        if result is None:
            return CallSummary(
                call_id=call_id,
                success=False,
                error="LLM call failed (MCP Router returned nothing).",
            )
    ...
    # _call_llm has been removed in favor of utils.mcp_router.MCPRouter
